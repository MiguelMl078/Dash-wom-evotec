...
def comprobacion_localidad(row): # Función para verificar si el código de localidad es correcto
    ...
    localidad_code = str(row["dane_cod_localidad"]) # Se extrae casilla con codigo DANE de localidad
...
def query_geodata(): 
    ...
    try:
        conn = mysql.connector.connect(**DBcredentials.BD_DESEMPENO_PARAMS) # Crear conexión a la base de datos

        # Crear un cursor
        cur = conn.cursor()

        query = """SELECT cell_name, site_name, band, sector_id, latitude, longitude, locality,  address AS dane_cod_localidad, city, dane_code, department, region 
            FROM ran_lte_baseline;"""
        ...
        df_geo = df_geo.dropna(subset="cell_name")
        df_geo['dane_cod_localidad'] = pd.to_numeric(df_geo['dane_cod_localidad'], errors='coerce')

        # Convertir valores válidos (no nulos) a enteros
        df_geo.loc[df_geo['dane_cod_localidad'].notnull(), 'dane_cod_localidad'] = df_geo.loc[
            df_geo['dane_cod_localidad'].notnull(), 'dane_cod_localidad'
        ].astype(int)
        ...
        df_geo["sector_name"] = df_geo["site_name"] + ": " + df_geo["sector"].astype(str)
        df_geo['AM'] = df_geo['cell_name'].apply(area_metro) # Función para columna de area metropolitana
        df_geo.rename(columns={
            'cell_name': 'cell_name_wom',
            'site_name': 'node_name',
            'latitude': 'latitud',
            'longitude': 'longitud',
            'locality': 'localidad',
            'city':'dane_nombre_mpio',
            'department':'dane_nombre_dpt',
            'region':'wom_regional'
        }, inplace=True)

        departamentos = gpd.read_file("co_2018_MGN_DPTO_POLITICO.geojson")
        departamentos["DPTO_CNMBR"] = departamentos["DPTO_CNMBR"].apply(lambda x: unidecode(x.lower()))
        # Crear un diccionario para el mapeo de nombre a código
        dpto_mapping = dict(zip(departamentos["DPTO_CNMBR"], departamentos["DPTO_CCDGO"]))

        #Eliminar tildes en la columna 'dane_nombre_dpt'
        df_geo["dane_nombre_dpt"] = df_geo["dane_nombre_dpt"].apply(lambda x: unidecode(x.lower()))

        # Crear la nueva columna con el código del departamento
        df_geo["dane_code_dpto"] = df_geo["dane_nombre_dpt"].map(dpto_mapping)
        # Convertir la columna 'dane_code_dpto' a enteros
        df_geo["dane_code_dpto"] = df_geo["dane_code_dpto"].astype(int)

        df_geo["dane_nombre_mpio"] = df_geo["dane_nombre_mpio"].str.upper() 
        df_geo['dane_nombre_dpt'] = df_geo['dane_nombre_dpt'].str.upper() 
        df_geo["node_name"] = df_geo["node_name"].str.upper() # Todo a mayusculas
        df_geo["sector_name"] = df_geo["sector_name"].str.upper() # Todo a mayusculas
        ...
        # Reemplazar los valores en la columna 'wom_regional'
        df_geo['wom_regional'] = df_geo['wom_regional'].replace(regional_mapping)
        df_geo.loc[df_geo['dane_nombre_dpt'].isin(["BOYACA", "TOLIMA", "HUILA"]), 'wom_regional'] = "Center"
        df_geo.loc[df_geo['dane_nombre_dpt'].isin(["CALDAS", "QUINDIO", "RISARALDA"]), 'wom_regional'] = "Northwest"
        df_geo.loc[df_geo['dane_nombre_dpt'] == "BOGOTA, D.C.", 'wom_regional'] = "Centro"

        # Cargar los datos del archivo GeoJSON
        geo_data = gpd.read_file("Clusterizacion.geojson")

        # Crear un GeoDataFrame con los datos del archivo GeoJSON
        geo_data = geo_data[['Nombre_Zona', 'key', 'geometry']]  # Ajustar columnas según archivo GeoJSON

        # Convertir las latitudes y longitudes a un GeoDataFrame
        df_geo['geometry'] = df_geo.apply(lambda row: Point(row['longitud'], row['latitud']), axis=1)
        gdf = gpd.GeoDataFrame(df_geo, geometry='geometry', crs=geo_data.crs)

        # Realizar la intersección espacial para encontrar el cluster correspondiente
        df_geo = gpd.sjoin(gdf, geo_data, how='left', predicate='intersects')

        # Agregar los nombres de las columnas necesarias
        df_geo.rename(columns={'key': 'cluster_key', 'Nombre_Zona': 'cluster_nombre'}, inplace=True)
        df_geo = df_geo.drop(columns=["geometry", "index_right"])
        return df_geo
...
#-----------------------------------Funciones Globales RAN--------------------------------------#
def run_global_ran():
    #---------- Importar Datos ----------#
    global df_geo,localidades,clusters, municipios, areas_metro, departamentos, regionales
    ...
    localidades = gpd.read_file("Localidades Crowdsourcing 2023/Crowdwourcing 2023/Localidades Finales mayo 18 v2.TAB", encoding='UTF-8')
    ...
    print("Funciones globales de RAN ejecutadas")



ran_layout = dbc.Container([
    dbc.Row([  # Primera fila con selección y gauge
        dbc.Col([
            dbc.Card([
                dbc.Row([
                    dbc.Col([
                ...
                ], className="mb-4", style={"padding": "1.3vh"}),
                dbc.Row([
                    dbc.Col([
                        dbc.Label("Tiempo"),
                ...
                ], className="mb-4", style={"padding": "1.1vh"}),
                dbc.Row([
                    dbc.Col(
                        dbc.Row([
                            dbc.Col(
                                dbc.Button("Reporte de celdas", id="reporte", n_clicks=0),
                                width="auto"),
                            dbc.Col(
                                dcc.Loading(dcc.Download(id="download_report"), color=MORADO_WOM),
                                width="auto"),
                        ]),
                        width=5, style={"textAlign": "left"}),
                    dbc.Col(
                        dbc.Row([
                            dbc.Col(
                                dbc.Button("Reporte de Agrupación", id="reporte_agrup", n_clicks=0),
                                width="auto"),
                            dbc.Col(
                                dcc.Loading(dcc.Download(id="download_report_agrup"), color=MORADO_WOM),
                                width="auto"),
                        ]),
                        width=5, style={"textAlign": "center"}),
                    dbc.Col(
                        dbc.Button("Buscar", id="solicitar", n_clicks=0),
                        width=2, style={"textAlign": "right"}),
                ],style={"height": "100%"}, align="center", justify="between")       
            ...
            dbc.Card([
                html.Div([
                    daq.Gauge(
                        ...
                        style={"width": "100%", "margin": "0 auto", "paddingBottom": "0px", "transform": "translateY(0px)"}  # Subimos el gauge
                    ),
                    html.Div(
                        "CAPACIDAD",
                        style={
                            ...
                            "marginTop": "-60px",  # Acercamos la palabra al gauge
                            "textAlign": "center"
                        }
                    )
                ], style={"textAlign": "center", "padding": "0"})  # Eliminamos cualquier padding adicional
            ], body=True, className="h-100 d-flex flex-column align-items-center justify-content-center")
        ...
        dbc.Col([
            dbc.Card([
                dcc.Loading( # Componente para mostrar estado de carga
                    children=[html.Div(id="test", children="Hola, haz una selección",  style={"height": "100%", "fontSize": "20px"})],
                    color="white"
                )
    ...
    dbc.Row([
        dbc.Col([
            dbc.Card([
                ...
                ], color=MORADO_WOM, type="circle", overlay_style={"visibility": "hidden", "height": "100%"})
            ], style={"height": "100%"})
        ], width=6, style={"height": "76vh", "padding": "0px"}),  # Eliminado padding
        
        dbc.Col([
            # Primera fila de dos gráficas
            dbc.Row([
                dbc.Col([
                    dbc.Card([
                        dcc.Graph(id="bh", style={"height": "38vh"})
                    ], style={"height": "100%"})
                ], width=6, style={"height": "38vh", "padding": "0px"}),  # Eliminado padding

                dbc.Col([
                    dbc.Card([
                        dcc.Graph(id="PRB", style={"height": "38vh"})
                    ], style={"height": "100%"})
                ], width=6, style={"height": "38vh", "padding": "0px"}),  # Eliminado padding
            ], className="g-0"),  # Eliminado espacio entre las columnas

            # Segunda fila de dos gráficas
            dbc.Row([
                dbc.Col([
                    dbc.Card([
                        dcc.Graph(id="traffic", style={"height": "38vh"})
                    ], style={"height": "100%"})
                ], width=6, style={"height": "38vh", "padding": "0px"}),  # Eliminado padding

                dbc.Col([
                    dbc.Card([
                        dcc.Graph(id="user_exp", style={"height": "38vh"})
                    ], style={"height": "100%"})
                ], width=6, style={"height": "38vh", "padding": "0px"}),  # Eliminado padding
            ], className="g-0"),  # Eliminado espacio entre las columnas
        ], width=6, style={"height": "76vh", "padding": "0px"})  # Eliminado padding
    ], className="g-0 mx-0", style={"height": "76vh", "margin": "0", "padding": "0"}),
    ...
    dbc.Card([
        ...
        dbc.Row([
            ...
            dbc.Col([
                dbc.Button(id="update_kpi", n_clicks=0, children="Update Map", className="me-2"),
                dbc.Button(id="fullscreen", n_clicks=0, children="Full Screen", className="me-2"),
                dbc.Button(id="download", n_clicks=0, children="Download"),
                dcc.Download(id="download_file"),
            ], width=6, align="center"),
        ], className="mb-3"),

        # Fila para el texto del mapa
        dbc.Row([
            dbc.Col([
                html.Div(id="map_text", children="No se ha seleccionado KPI en el mapa"),
            ], width=12)
        ], className="mb-3"),
   
        # Fila para la gráfica con altura ajustada
        dbc.Row([
            dbc.Col([
                dbc.Card([
                    dcc.Graph(id="graph_test", style={"height": "70vh"})  # Incrementa la altura de la gráfica
                ], style={"height": "70vh"})  # Ajusta la altura del contenedor
            ], style={"height": "70vh"})  # Ajusta la altura del contenedor en la columna
        ])
    ], body=True, className="g-0 mx-0")  # Clase para dar margen y padding al cuadro
], fluid=True, style={"minHeight": "100vh", "padding": "0px"})



def ran_callbacks(app):
    @app.callback(
    ...
    def update_dropdown(input):
        if input == "celda":
            options_df = df_geo["cell_name_wom"].copy() # Genero copia del df únicamente de la columna que contiene el nombre de las celdas
            ...
            options_df = df_geo[["localidad", "dane_cod_localidad", "dane_nombre_mpio", "dane_code"]].copy() # Copia solo columnas requeridas
            ...
            options_df = options_df.drop_duplicates(subset=["dane_cod_localidad"])

            options_df["CoLoc"] = options_df["dane_nombre_mpio"] + ": " + options_df["localidad"] + " " + options_df["dane_cod_localidad"].astype(str) # Nueva columna con nombre único de localidad
            options_df = options_df.sort_values(by=["CoLoc"]) # Organizo por nombre único
            options = [{'label': row["CoLoc"], 'value': row["dane_cod_localidad"]} for index, row in options_df.iterrows()]
    ...
    def make_zoom(input, agg):
        ...
        if agg == "celda":
            auxdf = df_geo[["cell_name_wom","latitud","longitud"]].copy() # Genero copia del df
            auxdf = auxdf[auxdf["cell_name_wom"] == input] # En el df busco la celda que concuerde con el valor del dropdown y guardo todas las filas
            lat_mean = auxdf["latitud"].astype(float).mean()
            lon_mean = auxdf["longitud"].astype(float).mean()
        elif agg == "sector":
            auxdf = df_geo[["sector_name","latitud","longitud"]].copy() # Genero copia del df
            ...
            auxdf = df_geo[["node_name","latitud","longitud"]].copy() # Genero copia del df
            ...
            lat_mean = auxdf["latitud"].astype(float).mean() # Promedio de coordenadas de todas las celdas que componen el grupo
            lon_mean = auxdf["longitud"].astype(float).mean()
        elif agg == "cluster":
            zoom = 13
            auxdf = df_geo[["cluster_key","latitud","longitud"]].copy() # Genero copia del df
            ...
            zoom = 12
            auxdf = df_geo[["node_name","dane_cod_localidad","latitud","longitud","dane_code"]].copy() # Genero copia del df
            auxdf = auxdf[auxdf["dane_cod_localidad"] == to_int(input)] # En el df busco la celda que concuerde con el valor del dropdown y guardo todas las filas
            ...
            zoom = 10
            auxdf = df_geo[["dane_code","latitud","longitud"]].copy() # Genero copia del df
            ...
            zoom = 10
            auxdf = df_geo[["AM","latitud","longitud"]].copy() # Genero copia del df
            ...
            zoom = 8
            auxdf = df_geo[["dane_code_dpto","latitud","longitud"]].copy() # Genero copia del df
            ...
            zoom = 6
            auxdf = df_geo[["wom_regional","latitud","longitud"]].copy() # Genero copia del df
    ...
    # Callback para descargar reporte de KPIs de cada celda dentro del rango de fechas
    @callback(
            Output(component_id='download_report_agrup', component_property='data'),

            Input(component_id='reporte_agrup', component_property='n_clicks'),
    ...
    def download_report_agreg(boton, start_date, end_date, agrup):
        if boton is None:  # Se debe presionar el botón para que se actualice el callback
            raise PreventUpdate
        ...
        try:
            ...
            if agrup == 'celda':
                ...
                tabla = f"ran_1h_{agrup}"  # Asegurar que la tabla esté correctamente interpolada
            ...
            if df.empty:  # Si el dataframe está vacío se levanta una excepción
                raise Exception("No se han retornado datos. Dataframe vacío")
            
            # Generar el archivo CSV
            file_name = f"KPI_Report_{tabla}_{start_date}-{end_date}.csv"
            return dcc.send_data_frame(df.to_csv, file_name, index=False)
    ...
    def map_kpi(boton, agg, kpi, start_date, end_date):
        ...
        if agg == "celda":
            cells = df_geo.drop_duplicates(subset=["cell_name_wom"]).copy() # Df con nombres únicos de celda
            bh_df[data_column] = bh_df[data_column].str.upper() # Valores a mayusculas
            df_merged = cells.merge(bh_df, how="left", left_on="dcell_name_wom", right_on="Cell_name") # Merge según nombre de celdas

            fig = px.scatter_mapbox(df_merged, lat="latitud", lon="longitud",
                                    color=graph_column,
                                    ...
                                    zoom=map_layout["zoom"], center=map_layout["center"],
                                    hover_name="cell_name_wom",
                                    custom_data="cell_name_wom"
                                    )
